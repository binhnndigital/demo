"""PNG teacher certificate generation module - Bolt.now / PSU"""
import random
...
def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    ...
    try:
        ...
        try:
            ...
            # Wait for image loading (if external images exist)
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Batch concurrent generation of multiple PNGs (performance optimized)
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Generate two PNGs: Teacher card + Employment certificate (concurrent optimized)
