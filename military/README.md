...
The ChatGPT military verification process differs from regular student/teacher verification, requiring an additional API call to collect military status information before submitting the personal information form.

## 🔄 Verification Process

### Step 1: Collect Military Status (collectMilitaryStatus)

Before submitting the personal information form, this API must be called to set the military status.
...
```json
{
    "status": "VETERAN" // 3 options total
}
```
...
### Step 2: Collect Inactive Military Personal Information (collectInactiveMilitaryPersonalInfo)

Submit personal information using the `submissionUrl` returned from step 1.

**Request Information**:
- **URL**: Obtained from step 1's `submissionUrl` response
...
- `organization`: Military organization information (see organization list below)
...
Below are available military organization options:
...
1. **Must Execute in Order**: Must first call `collectMilitaryStatus` to get `submissionUrl`, then call `collectInactiveMilitaryPersonalInfo`
2. **Organization Information**: The `organization` field needs to include both `id` and `name`, can be randomly selected from the list above or let user choose
3. **Date Format**: `birthDate` and `dischargeDate` must use `YYYY-MM-DD` format
4. **Metadata**: The `submissionOptIn` in the `metadata` field contains privacy policy consent text, needs to be extracted from original request or constructed
...
- [ ] Generate compliant personal information (name, birth date, email, etc.)
