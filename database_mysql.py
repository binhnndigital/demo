...
Use provided MySQL server for data storage
"""
import logging
...
class MySQLDatabase:
    ...
    def __init__(self):
        ...
        # Read configuration from environment variables (recommended) or use default values
        self.config = {
    ...
    def create_card_key(
        ...
        try:
            ...
            logger.error(f"Code already exists: {key_code}")
            conn.rollback()
            return False
        except Exception as e:
    ...
    def use_card_key(self, key_code: str, user_id: int) -> Optional[int]:
        ...
        try:
            ...
            logger.error(f"Failed to use code: {e}")
            conn.rollback()
            return None
        finally:
            cursor.close()
    ...
    def get_all_card_keys(self, created_by: Optional[int] = None) -> List[Dict]:
        """Get all codes (can filter by creator)"""
        conn = self.get_connection()
        cursor = conn.cursor(DictCursor)
...
# 创建全局实例的别First name，保持与SQLite版本的兼容性
