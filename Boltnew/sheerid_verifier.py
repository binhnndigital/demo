...
class SheerIDVerifier:
    ...
    def create_verification(self) -> str:
        ...
        logger.info(f"✅ Obtained verificationId: {self.verification_id}")
    ...
    def verify(
        ...
        """Execute teacher verification process"""
        try:
            ...
            logger.info("Step 1/5: Generate teacher PNG documents...")
            ...
            # Submit teacher info
            logger.info("Step 2/5: Submit teacher info...")
            step2_body = {
            ...
            logger.info(f"✅ Step 2 completed: {getattr(step2_data, 'get', lambda k, d=None: d)('currentStep')}")
            current_step = (
            ...
            if current_step in ["sso", "collectTeacherPersonalInfo"]:
                logger.info("Step 3/5: Skip SSO verification...")
                step3_data, _ = self._sheerid_request(
                    "DELETE",
                    f"{config.SHEERID_BASE_URL}/rest/v2/verification/{self.verification_id}/step/sso",
                )
                logger.info(f"✅ Step 3 completed: {getattr(step3_data, 'get', lambda k, d=None: d)('currentStep')}")
                current_step = (
                    step3_data.get("currentStep", current_step) if isinstance(step3_data, dict) else current_step
                )

            # Request upload and upload documents
            logger.info("Step 4/5: Request upload URL ...")
            step4_body = {
                "files": [
                    {
            ...
            if len(documents) != len(assets):
                raise Exception("Returned upload task count does not match file count")
            ...
            logger.info(f"✅ Document submission completed: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

            # Get final status (including rewardCode)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Enter SheerID verification entry link (with externalUserId): ").strip()
